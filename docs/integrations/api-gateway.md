...
В следующем изменении `api-gateway` должен получить `TASKS_IT_URL`, отдельный HTTP-клиент и GraphQL adapters без бизнес-логики.
...
JWT context преобразуется в `X-User-ID` и `X-User-Roles` для каждого защищённого вызова. `X-Request-ID` также передаётся дальше. Ответы и error codes `tasks-it` маппятся в GraphQL errors без раскрытия upstream body или внутренних адресов.
...
Для формы создания теста `api-gateway` отдельно запрашивает темы у `entities`. `tasks-it` не проксирует этот список.
