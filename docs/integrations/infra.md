...
`infra` должен добавить отдельную PostgreSQL database `tasks`, deployment и internal service `tasks:8080`. База не должна разделяться таблицами с другими сервисами.
...
Kafka должна содержать topics `code-execution.requests.v1` и `code-execution.results.v1`. В Docker Compose `KAFKA_BOOTSTRAP_SERVERS` указывает на internal listener `kafka:9092`; локальный запуск с host использует advertised host listener, например `localhost:29092`. Request topic читает `sandbox`, result topic — consumer group `tasks-code-results-v1`.

Readiness probe использует `/ready` и требует доступности PostgreSQL и Kafka, liveness probe — `/health`. `tasks` доступен только `api-gateway`, не ingress.
